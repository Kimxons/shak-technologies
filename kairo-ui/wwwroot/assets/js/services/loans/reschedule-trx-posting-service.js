(function (global) {
  ...
  const RescheduleTrxPostingService = {
    ...
    async calculateLocalAmount(amount, exchangeRate) {
      try {
        // Calculate local amount directly
        const localAmount = (parseFloat(amount) || 0) * (parseFloat(exchangeRate) || 1.0);
        return {
          localAmount,
          success: true
        };
     ...
     */
    async calculateForexGainLoss(transactionAmount, exchangeRate, meanRate, transactionType, accountCurrency, localCurrency) {
      // JS implementation of C# CalculateGainLoss
      // transactionType: expects 'CR' or 'DR' (map as needed by caller)
      // accountCurrency/localCurrency: pass as string (e.g. 'ETB', 'KES')
      try {
        transactionAmount = parseFloat(transactionAmount) || 0;
        exchangeRate = parseFloat(exchangeRate) || 1.0;
        meanRate = parseFloat(meanRate) || 1.0;
        let gainLoss = 0;
        if (transactionType === "CR") {
          if (accountCurrency !== localCurrency)
            gainLoss = transactionAmount * (exchangeRate - meanRate); // Sell
          else
            gainLoss = transactionAmount * (meanRate - exchangeRate); // Buy
        } else {
          if (accountCurrency !== localCurrency)
            gainLoss = transactionAmount * (meanRate - exchangeRate); // Buy
          else
    ...
    async addLoanReschPostTrx(params) {
      try {
        ...
        const payload = {
          ...
          CreatedOn: params.createdOn || new Date().toISOString().replace('T', ' ').substring(0, 19),
        ...
        console.log('[RescheduleTrxPostingService] DetailRecords XML:', detailRecordsXml);
        console.log('[RescheduleTrxPostingService] DB Call: exec p_AddLoanReschPostTrx', {
          ...
          TransactionCount: params.transactions?.length || 0
        });
    ...
    async getTillDetails() {
      try {
        const payload = {
          ...
          CashierID: "MARTIN_MARANGA" // Hardcoded for testing - replace with dynamic operator ID if needed
        };
    ...
    /**
     * DATABASE CALL #6: SAVE RESCHEDULE LOAN POST TRX
     * Alias for addLoanReschPostTrx - saves transaction posting to database
     * exec p_AddLoanReschPostTrx @OurBranchID, @AccountID, @LoanSeries, @DetailRecords, etc.
     */
    async saveReschLoanPostTrx(params) {
      return await this.addLoanReschPostTrx(params);
    }
  };
