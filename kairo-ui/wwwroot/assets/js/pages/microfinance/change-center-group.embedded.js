(function () {
      const form = document.getElementById('change-center-group-form');
      if (!form) return;

      form.querySelectorAll('[data-ccg-lookup]').forEach((btn) => {
        btn.addEventListener('click', () => {
          const which = btn.getAttribute('data-ccg-lookup');
          window.alert('Lookup (' + which + ') is a UI stub in this prototype.');
        });
      });

      form.querySelectorAll('[data-ccg-action]').forEach((btn) => {
        btn.addEventListener('click', () => {
          const type = btn.getAttribute('data-ccg-action');
