...
async function validateClientIdFromInput({ silentOnBlank = false } = {}) {
    ...
    if (!window.ClientService || typeof window.ClientService.validateClient360 !== 'function') {
        client360Toast('ClientService.validateClient360 is not available.', 'error');
        return { ok: false, name: '' };
    }
    ...
    try {
        const resp = await window.ClientService.validateClient360({
            OurBranchID: ctx.OurBranchID,
        ...
        // CoreApi.post returns a normalized contract: { success, code, message, data, Details }
        if (resp && resp.success === false) {
            client360Toast(resp.message || 'Client validation failed.', 'error');
            return { ok: false, name: '' };
        }
...
async function handleClientSearch() {
    ...
    if (!window.ClientService || typeof window.ClientService.searchClients !== 'function') {
        client360Toast('ClientService.searchClients is not available.', 'error');
        return;
    }
    ...
    if (!clientLookupModal) {
        clientLookupModal = new window.SearchModal({
            ...
            searchFn: async (payload) => {
                ...
                return window.ClientService.searchClients({ RequestData: requestData });
            },
            onError: (err) => {
...
async function handleViewClient() {
    ...
    if (!window.ClientService || typeof window.ClientService.viewClient360 !== 'function') {
        client360Toast('ClientService.viewClient360 is not available.', 'error');
        return;
    }
    ...
    try {
        ...
        const resp = await window.ClientService.viewClient360({
            OurBranchID: ctx.OurBranchID,
            ClientID: clientId,
            OperatorID: ctx.OperatorID
        });
        ...
        if (!resp || resp.success === false) {
            client360Toast(resp?.message || 'Failed to load client 360', 'error');
            return;
        }

        const payload = resp.raw || resp.data || resp;

        const responseCode = String(payload?.ResponseCode ?? resp?.code ?? '').trim();
        if (responseCode && responseCode !== '00') {
            client360Toast(payload?.ResponseMessage || resp?.message || 'Failed to load client 360', 'error');
            return;
        }
