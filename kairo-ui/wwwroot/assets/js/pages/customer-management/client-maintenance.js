(function (global) {
  ...
  const readNumber = (value, fallback = 0) => {
    // Handle comma-formatted values (e.g., "10,000.50")
  ...
  class ClientMaintenancePage {
    constructor() {
     ...
     this.stepper = new Stepper(this.form.querySelector("[data-stepper]"));
      ...
      // Search modal instances (initialized via initializeSearchModals)
      this.clientSearchModal = null;
      this.applicationSearchModal = null;
      this.glAccountSearchModal = null;
    ...
    setPageFunction(mode) {
      ...
      // Re-render products/services to update checkbox enabled states based on new mode
      this.renderProducts();
      this.renderServices();
    }
    ...
    /**
     * Initialize search modals using the global SearchModal component
     * Replaces custom lookup modals with standardized pattern
     */
    initializeSearchModals() {
      const self = this;
      
      // Helper to wait for SearchModal to be available
      const waitForSearchModal = (callback, maxWaitMs = 5000, intervalMs = 100) => {
        ...
        (function poll() {
          if (window.SearchModal) {
            ...
            console.warn("[ClientMaintenance] SearchModal not available after waiting");
          }
        })();
      };
      ...
      waitForSearchModal(() => {
        const moduleID = 1000;
        const getOperatorId = () => self.session?.operatorId || self.session?.name || "web_portal";
        const getOurBranchId = () => {
          const envBranchId = window.Environment?.OurBranchID || window.Environment?.ourBranchId || "";
          return self.session?.branchID || self.session?.branchId || self.session?.BranchID || envBranchId || "0603";
        };
        
        // Client Search Modal
        self.clientSearchModal = new window.SearchModal({
          prefix: 'cm-client-search',
          moduleID,
          getOperatorId,
          getOurBranchId,
          onError: (msg) => {
            console.error("[ClientMaintenance] Client search error:", msg);
            self.showToast("Search error occurred", "danger");
          }
        });
        
        // Application Search Modal (WFClientID)
        self.applicationSearchModal = new window.SearchModal({
          prefix: 'cm-app-search',
          moduleID,
          getOperatorId,
          getOurBranchId,
          onError: (msg) => {
            console.error("[ClientMaintenance] Application search error:", msg);
            self.showToast("Search error occurred", "danger");
          }
        });
        
        // GL Account Search Modal
        self.glAccountSearchModal = new window.SearchModal({
          prefix: 'cm-gl-search',
          moduleID,
          getOperatorId,
          getOurBranchId,
          onError: (msg) => {
            console.error("[ClientMaintenance] GL search error:", msg);
            self.showToast("Search error occurred", "danger");
          }
        });
        
        console.log("[ClientMaintenance] Search modals initialized");
      });
    }

    /**
     * Open the client search modal
     * @param {string} prefillValue - Optional value to prefill in search
     * @param {Function} customOnSelect - Optional custom handler for selection (for relations/documents)
     */
    openClientSearchModal(prefillValue = "", customOnSelect = null) {
      ...
      this.clientSearchModal.open({
        ...
        tableID: 'clientId',
        searchFields: [
          { name: 'ClientID', label: 'Client ID', column: 'ClientID', value: prefillValue },
        ...
        onSelect: (record) => {
          if (customOnSelect) {
            customOnSelect(record);
          } else {
            self.applyLookupSelection(record);
          }
        }
      });
    }
    ...
    /**
     * Open the application search modal (WFClientID)
     * @param {string} prefillValue - Optional value to prefill
     */
    openApplicationSearchModal(prefillValue = "") {
      ...
      const clientType = clientTypeField?.value || "I";
      ...
      this.applicationSearchModal.open({
        ...
        tableID: tableID,
        whereStmt: `WFClientStatusID = 'P'`,
        searchFields: [
          { name: 'ClientID', label: 'Request ID', column: 'ClientID', value: prefillValue },
          { name: 'Name', label: 'Client Name', column: 'Name' }
        ],
        autoSearch: true,
        onSelect: (record) => {
          self.applyApplicationSelection(record);
        }
      });
    }

    /**
     * Open the GL Account search modal
     * @param {string} prefillValue - Optional value to prefill
     */
    openGLAccountSearchModal(prefillValue = "") {
      if (!this.glAccountSearchModal) {
        this.showToast("Search modal not ready, please try again.", "warning");
        return;
      }
      
      const self = this;
      this.glAccountSearchModal.open({
        title: 'Find GL Account',
        tableID: 'GeneralLedgerID',
        searchFields: [
          { name: 'AccountID', label: 'Account ID', column: 'AccountID', value: prefillValue },
          { name: 'Description', label: 'Description', column: 'Description' }
        ],
        autoSearch: true,
        onSelect: (record) => {
          self.applyGLAccountSelection(record);
        }
      });
    }
    ...
    syncActionButtons() {
      ...
      // Stepper navigation buttons: disable Next buttons in View mode
      // (to prevent accidental saves when just viewing data)
      const isViewMode = this.state.pageFunction === "View";
      this.form.querySelectorAll('[data-stepper-action="next"]').forEach((btn) => {
    ...
    bindEvents() {
      ...
      const rmFields = this.form.querySelectorAll('[name="RelationshipManagerID"]');
      ...
      const searchButton = this.form.querySelector("[data-client-search]");
        ...
        this.openClientSearchModal(forceLookupModal ? typedClientId : "");
      });
      ...
      if (appSearchButton) {
        appSearchButton.addEventListener("click", (event) => {
          ...
          this.openApplicationSearchModal("");
        });
      }
    ...
    applyGLAccountSelection(record) {
      ...
      // Set the GL Account Name field
      const glNameField = document.getElementById("corpReportingGLAccountName");
      if (glNameField) {
        glNameField.value = record.Description || record.AccountName || "";
      }
    ...
    /**
     * Look up GL Account name by AccountID and populate the name field
     */
    async lookupGLAccountName(accountId) {
      if (!accountId) return;
      const searchSvc = this.searchService || window.SearchService;
      if (!searchSvc?.search) return;
      ...
      try {
        ...
        if (results.length > 0) {
          const glNameField = document.getElementById("corpReportingGLAccountName");
          if (glNameField) {
            glNameField.value = results[0].Description || results[0].ShortName || "";
          }
        }
      } catch (error) {
        console.warn("[lookupGLAccountName] Unable to fetch GL name:", error);
      }
    }
    ...
    async loadPipelineApplication(requestId) {
      ...
      try {
        ...
        // 7) Photos/Signatures (use TempImageService like loadClientViaSteppers)
        const TempImageSvc = global.TempImageService;
        if (TempImageSvc) {
          tasks.push(
            TempImageSvc.getClientImages(requestId).then((resp) => {
    ...
    openLookupForRelatedClient(prefill = "") {
      const self = this;
      this.openClientSearchModal(prefill, (record) => {
        const root = self.form.querySelector("[data-relations-form]");
        ...
        if (root && related) {
          ...
          self.hydrateRelationFormFromRelatedClientId(related);
        }
      });
    }
    ...
    openLookupForDocumentReceiver(prefill = "") {
      const self = this;
      this.openClientSearchModal(prefill, (record) => {
        const root = self.form.querySelector("[data-documents-form]");
    ...
    async saveStepDraft(activeStep) {
      ...
      const saveIndividual = async () => {
        ...
        const data = useUpdate
          ? {
            ...
            AgeAsOn: "",
            BloodGroupID: this.form.elements.BloodGroup?.value || "",
            ...
            PassportIssueDate: this.form.elements.IDIssueDate?.value || "",
      ...
      const saveCorporate = async () => {
        ...
        const base = {
          ...
          BusinessStartedYear: getVal("corpYearStarted"),
          NoOfEmployees: getVal("corpEmployees"),
    ...
    applyBasicDetailsStepper(details) {
      ...
      // RelationshipManagerID - set on both Personal and Corporate steppers
      // The value comes from BasicDetails and needs to be propagated to both selects
      const rmValue = details.RelationshipManagerID || "";
      if (rmValue) {
        const personalRM = document.getElementById("relationshipManager");
        const corpRM = document.getElementById("corpRelationshipManager");
        if (personalRM) {
          personalRM.value = rmValue;
          personalRM.dispatchEvent(new Event('change', { bubbles: true }));
        }
        if (corpRM) {
          corpRM.value = rmValue;
    ...
    applyIndividualStepper(row) {
      ...
      // Blood Group - form uses name="BloodGroup" but API returns BloodGroupID
      setFormFieldValue(this.form, "BloodGroup", row.BloodGroupID);
      ...
      // Identification type - use ID selector since multiple elements share name="IdentificationTypeID"
      const idTypeSelect = this.form.querySelector("#idType");
      if (idTypeSelect && row.IdentificationTypeID) {
        idTypeSelect.value = row.IdentificationTypeID;
      }

      // Identification dates/number
    ...
    applyCorporateStepper(row, basicDetails = null) {
      ...
      setById("corpYearStarted", row.BusinessStartedYear);
      setById("corpEmployees", row.NoOfEmployees);
      ...
      // Look up GL Account name if we have an ID
      if (row.ReportingGLID) {
        this.lookupGLAccountName(row.ReportingGLID);
      }
    ...
    populateClientSummary(payload, fallbackId) {
      ...
      const rmFields = this.form.elements.RelationshipManagerID;
      const rmField = rmFields?.length ? rmFields[0] : rmFields; // Get first element if collection
      const rmValue = payload?.RelationshipManagerID ?? payload?.RelationshipManager ?? "";
    ...
    async handleSubmit(action = "save") {
      ...
      const submitBtns = document.querySelectorAll('[data-submit-action="save"], [data-submit-action="approve"]');
