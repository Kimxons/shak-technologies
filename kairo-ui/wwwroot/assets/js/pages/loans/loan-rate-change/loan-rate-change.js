(function (global) {
  ...
  // ========================================
  function updateScheduleEmptyMessages() {
    // Current Schedule
    const currentEmptyMsg = document.querySelector('[data-lrc-current-empty][data-empty-message]');
    const currentRows = elements.currentScheduleBody && elements.currentScheduleBody.children.length;
    if (currentEmptyMsg) {
      currentEmptyMsg.style.display = (currentRows > 0) ? 'none' : '';
    }
    // New Schedule
    const newEmptyMsg = document.querySelector('[data-lrc-new-empty][data-empty-message]');
    const newRows = elements.newScheduleBody && elements.newScheduleBody.children.length;
    if (newEmptyMsg) {
      newEmptyMsg.style.display = (newRows > 0) ? 'none' : '';
    }
  }
