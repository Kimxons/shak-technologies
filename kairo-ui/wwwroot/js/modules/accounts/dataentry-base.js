...
const DataEntryBase = (function() {
   ...
   * @param {boolean} skipUnsavedCheck - Skip unsaved changes check
   */
  function closeChildForm(skipUnsavedCheck = false) {
    // Dispatch a cancelable close event
    const closeEvent = new CustomEvent('dataentry:beforeclose', {
      bubbles: true,
      cancelable: true
    });
    
    const shouldClose = document.dispatchEvent(closeEvent);
    
    if (!shouldClose && !skipUnsavedCheck) {
  ...
  function initWindowControls() {
    const deWindow = document.querySelector('.de-window') || document.querySelector('.window');
    
    // Wire all title bar buttons by data-action attribute (support both am-btn and de-title-btn)
  ...
  /**
   * Lookup configuration matched with System Search configurations
   * Maps input field IDs to lookup table IDs
   */
  const LOOKUP_CONFIG = {
    // Standard field names (camelCase used in child pages)
    ...
    'requestRef': { tableID: 'RequestID', keyField: 'RequestID', nameField: 'Description' },
    'referenceId': { tableID: 'ReferenceID', keyField: 'ReferenceID', nameField: 'Description' },
  ...
  /**
   * Wire up all lookup buttons in the current document
   * Uses SearchModal for the lookup popup
   */
  function wireLookups() {
    const lookupBtns = document.querySelectorAll('.btn-lookup');
    if (lookupBtns.length === 0) return;

    // Ensure SearchModal and AppCore are available
    if (typeof window.SearchModal === 'undefined' || typeof window.AppCore === 'undefined') {
      console.warn('[DataEntryBase] SearchModal or AppCore not loaded. Lookups disabled.');
      return;
    }
    ...
    lookupBtns.forEach(btn => {
      ...
      btn.addEventListener('click', function(e) {
        ...
        // Determine target input ID from data-target-input or data-lookup attribute
        const targetInputId = this.dataset.targetInput || this.dataset.lookup;
        if (!targetInputId) {
          console.warn('[DataEntryBase] Lookup button missing data-target-input or data-lookup attribute');
          return;
        }

        // Get configuration for this lookup type
        const baseConfig = LOOKUP_CONFIG[targetInputId] || {
          tableID: targetInputId.replace(/Id$/i, '').replace(/ID$/i, ''),
          keyField: targetInputId,
          nameField: targetInputId.replace(/Id$/i, 'Name').replace(/ID$/i, 'Name')
        };

        // Shallow copy to allow dynamic filtering
        const config = { ...baseConfig };

        // Dynamic filtering logic for Account lookups
        if (config.tableID === 'AccountID') {
          const branchInput = document.getElementById('branchId') || document.getElementById('BranchID');
          const clientInput = document.getElementById('clientId') || document.getElementById('ClientID');

          let whereParts = [];
          if (branchInput && branchInput.value) {
            whereParts.push(`OurBranchID = '${branchInput.value}'`);
          }
          if (clientInput && clientInput.value) {
            whereParts.push(`ClientID = '${clientInput.value}'`);
          }
        ...
        console.log(`[DataEntryBase] Opening lookup for ${targetInputId}`, config);

        // Create a new SearchModal instance for fresh state
        ...
        searchModal.open({
          ...
          onSelect: (selectedRow) => {
            ...
            console.log('[DataEntryBase] Lookup selected:', selectedRow);
            ...
            // Find the ID input - try exact match first, then case-insensitive
            const idInput = document.getElementById(targetInputId) ||
              document.querySelector(`input[name="${targetInputId}"]`);

            // Find the Name input - typically sibling or nearby element
            // Try common naming patterns: branchId -> branchName, BranchID -> BranchName
            const nameInputId = targetInputId.replace(/Id$/i, 'Name').replace(/ID$/i, 'Name');
    ...
    console.log(`[DataEntryBase] Wired ${lookupBtns.length} lookup buttons`);
  }
