...
window.SignaturePhotoModule = (function () {
    ...
    const API = {
        ...
        GET_SIGNATURE: 'AccountsMaintenance/api/get-signature-image',
        GET_PHOTO: 'AccountsMaintenance/api/get-photo-image'
    };
    ...
    async function selectSignatory(signatory, index) {
        ...
        // Load signature and photo images
        await loadSignatureImage(signatory);
        await loadPhotoImage(signatory);
    ...
    async function loadSignatureImage(signatory) {
        const container = el('div_signatureImage');
        if (!container) return;
        ...
        try {
            ...
            const payload = {
                OurBranchID: ctx.OurBranchID,
                SignatoryID: signatory.SignatoryID || signatory.ClientID,
                OperatorID: ctx.OperatorID
            };

            const result = await AppCore.invokeControllerAsync(API.GET_SIGNATURE, payload);
            ...
            if (result?.success && result?.imageData) {
                ...
                container.innerHTML = '<span class="text-muted">No signature available</span>';
            }
        } catch (error) {
            console.warn('[SignaturePhoto] Error loading signature:', error);
            container.innerHTML = '<span class="text-muted">No signature available</span>';
        }
    }

    // ── Load Photo Image ───────────────────────────────────────
    async function loadPhotoImage(signatory) {
        const container = el('div_photoImage');
        if (!container) return;
        ...
        try {
            ...
            const result = await AppCore.invokeControllerAsync(API.GET_PHOTO, payload);
            ...
            if (result?.success && result?.imageData) {
                ...
                container.innerHTML = '<span class="text-muted">No photo available</span>';
            }
        } catch (error) {
            console.warn('[SignaturePhoto] Error loading photo:', error);
            container.innerHTML = '<span class="text-muted">No photo available</span>';
        }
    }
