(function () {
  ...
  const printBtn = document.querySelector('[data-ach-print]');
  const backBtn = document.querySelector('[data-ach-back]');
  ...
  navButtons.forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (target === 'asset-cost-history') return;
      if (target === 'depreciation-schedule') {
        navigateTo('DepreciationSchedule.html');
      }
    });
  });
  ...
  const closeParentModalIfPresent = () => {
    try {
      ...
      const modalEl = parentWin.document?.getElementById('assetCostHistoryModal');
