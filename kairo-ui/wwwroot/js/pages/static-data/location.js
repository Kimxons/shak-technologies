(() => {
  ...
  function applyDataToForm(data) {
    ...
    if (name != null) qs("#LocationDesc") && (qs("#LocationDesc").value = String(name));
  ...
  function setMode(nextMode, { initial = false } = {}) {
    ...
    // Make LocationDesc editable in add/update modes
    if (isFormEditable) {
      qs("#LocationDesc")?.removeAttribute("readonly");
    } else {
      qs("#LocationDesc")?.setAttribute("readonly", "");
    }
  ...
  async function handleSave() {
    ...
    const name = qs("#LocationDesc")?.value?.trim() || "";
    if (!name) {
