  /**
   * Populate a dropdown with options (matches Loan Maintenance logic)
...
(function () {
  ...
  function initializeSearchModal() {
    ...
    searchModal = new SearchModal({
      // Reason dropdown initialization is now handled by initializeAllDropdowns from the service
  ...
  async function initializeReasonDropdown() {
    console.log('[LoanDisbursementReversal] initializeReasonDropdown called');
    let options = [];
    if (window.LoanDisbursementReversalService && typeof window.LoanDisbursementReversalService.getLoanReversalReasonOptions === 'function') {
      try {
        console.log('[LoanDisbursementReversal] Calling getLoanReversalReasonOptions...');
        options = await window.LoanDisbursementReversalService.getLoanReversalReasonOptions();
        console.log('[LoanDisbursementReversal] getLoanReversalReasonOptions returned:', options);
      } catch (err) {
        console.error('[LoanDisbursementReversal] Failed to fetch LoanReversalReasonID options:', err);
      }
    } else {
      console.error('[LoanDisbursementReversal] LoanDisbursementReversalService or getLoanReversalReasonOptions not available');
    }
    populateDropdown(elements.reasonID, options);
  }
  ...
  function setFormMode(mode) {
    ...
    switch (mode) {
      ...
      case 'EDIT':
        ...
        // Always repopulate Reason dropdown when entering edit mode
        if (typeof initializeReasonDropdown === 'function') {
