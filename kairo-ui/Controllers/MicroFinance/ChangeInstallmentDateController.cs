...
namespace kairo_ui.Controllers.MicroFinance
{
    ...
    public class ChangeInstallmentDateController : Controller
    {
        ...
        [HttpPost]
        [Route("old-api")]
        public async Task<IActionResult> PostOldApi([FromBody] ChangeInstallmentDateOldApiRequest request)
        {
            try
            {
                if (!_authService.IsAuthenticated())
                {
                    _logger.LogWarning("Unauthenticated Change Installment Date OldAPI attempt");
                ...
                _logger.LogInformation("Change Installment Date OldAPI request for {FormId}: {Request}",
            ...
            {
                _logger.LogError(ex, "Error processing Change Installment Date OldAPI request");
    ...
    public class ChangeInstallmentDateOldApiRequest
    {
