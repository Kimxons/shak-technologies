...
(async function() {
  ...
  class ClientApprovalController {
    constructor() {
      ...
      // Wire up title bar - wait for KairoTitleBar component to initialize
      setTimeout(() => {
        this.wireTitleBar();
      }, 100);
    }
    ...
    /**
     * Wire up KairoTitleBar component
     */
    wireTitleBar() {
      // Find the title bar container or the actual title bar element
      const titleBarContainer = document.querySelector('[data-kairo-titlebar]');
      const titleBarElement = document.querySelector('.ktb-title-bar');
      
      if (!titleBarContainer && !titleBarElement) {
        console.warn('[ClientApproval] Title bar not found, will retry');
        // Retry after a short delay in case component hasn't initialized yet
        setTimeout(() => this.wireTitleBar(), 100);
        return;
      }

      // Use the actual title bar element if available, otherwise use container
      const targetElement = titleBarElement || titleBarContainer;

      // Listen for refresh event from KairoTitleBar component
      targetElement.addEventListener('kairo:titlebar:refresh', () => {
        console.log('[ClientApproval] Refresh event received from title bar');
        // Reload pending approvals if filters are set
        if (this.elements.branchIdValue.value && 
            this.elements.clientTypeFilter.value && 
            this.elements.applicationIdValue.value) {
      ...
      targetElement.addEventListener('kairo:titlebar:maximize', (e) => {
        const isMaximized = e.detail && e.detail.maximized;
        console.log('[ClientApproval] Maximize event received, maximized:', isMaximized);
        
        // The component already handles CSS class toggle, but ensure backward compatibility
        ...
        // Toggle sidebar in parent window
        // When maximized (true): closes/collapses sidebar drawer
        // When restored (false): opens/expands sidebar drawer
      ...
      targetElement.addEventListener('kairo:titlebar:close', () => {
        console.log('[ClientApproval] Close event received from title bar');
        this.postClose();
      });

      // Also listen for the default dataentry:refresh event for backward compatibility
      document.addEventListener('dataentry:refresh', () => {
        console.log('[ClientApproval] dataentry:refresh event received');
        if (this.elements.branchIdValue.value && 
            this.elements.clientTypeFilter.value && 
            this.elements.applicationIdValue.value) {
          this.loadPendingApprovals();
      ...
      console.log('[ClientApproval] Title bar wired up successfully');
    }
    ...
    async handleApprove() {
      ...
      try {
        ...
        if (approvalResponse.success) {
          ...
          // Push to supervision for each approved client
          try {
            const supervisionPromises = this.selectedClients.map(async (selected) => {
              // Get client details for NewData/OldData
              ...
              // Build keys in the required format: [OperatorID:<value>][ClientID:<value>]
              const searchKey = `[OperatorID:${operatorId}][ClientID:${selected.ClientID}]`;
              const lockKey = `[OperatorID:${operatorId}][ClientID:${selected.ClientID}]`;
