(() => {
  ...
  function renderVendorSearchRows(rows) {
    ...
    for (const r of rows) {
      ...
      tr.innerHTML = `
        ...
        <td class="text-end"><button type="button" class="btn btn-sm btn-primary" data-vendor-select="1">Select</button></td>
      `;
      frag.appendChild(tr);
    }
  ...
  function wireVendorSearchPanel() {
    ...
    results?.addEventListener("click", (e) => {
      const selectBtn = e.target?.closest?.("button[data-vendor-select]");
      if (selectBtn) {
        const tr = selectBtn.closest?.("tr[data-vendor-id]");
  ...
  function applyDataToForm(row) {
    ...
    if (name != null) qs("#Name") && (qs("#Name").value = String(name));
  ...
  function buildSavePayload() {
    ...
    const vendorName = qs("#Name")?.value?.trim() || "";
    if (!vendorName) throw new Error("Name is required");
    ...
    const existingCreatedBy = qs("#CreatedBy")?.value?.trim() || "";
    const existingCreatedOn = qs("#CreatedOn")?.value?.trim() || "";
    const existingModifiedOn = qs("#ModifiedOn")?.value?.trim() || "";
    ...
    const payload = {
      ...
      VendorID: vendorId,
      // Stored proc signature requires @VendorName in some deployments.
      ...
      // Some deployments use legacy Phone1/Phone2 proc parameters.
      Phone1: qs("#PhoneHome")?.value?.trim() || "",
      Phone2: qs("#PhoneWork")?.value?.trim() || "",
      Mobile: qs("#Mobile")?.value?.trim() || "",
      // Some deployments use legacy Email proc parameter.
      Email: qs("#EmailId")?.value?.trim() || "",
      // Some deployments use legacy Fax proc parameter.
      Fax: qs("#FaxNo")?.value?.trim() || "",
      // Some deployments use ContactPersonID proc parameter.
      ContactPersonID: qs("#ContactPerson")?.value?.trim() || "",
      // Some deployments also require ContactPersonName.
      ContactPersonName: qs("#ContactPersonName")?.value?.trim() || "",
      // Some deployments use legacy ProductDetails proc parameter.
      ProductDetails: qs("#ProductsDealingWith")?.value?.trim() || "",

      // Backend contract uses audit operator fields rather than OperatorID.
      // For UPDATE, preserve CreatedBy/CreatedOn from the loaded record to avoid concurrency/ownership issues.
      CreatedBy: state.mode === MODES.ADD ? operatorId : existingCreatedBy,
      CreatedOn: state.mode === MODES.ADD ? "" : existingCreatedOn,
      ModifiedBy: operatorId,
      ModifiedOn: state.mode === MODES.ADD ? "" : existingModifiedOn,
      // Proc requires SupervisedBy in this environment; default to current operator if blank.
      SupervisedBy: (qs("#SupervisedBy")?.value?.trim() || operatorId),
  ...
  function minimizeVendorSavePayload(payload, level = 1) {
    ...
    const keepLevel2 = new Set([
      // Level 2: data-only payload (helps when proc rejects audit fields).
      "BankID",
      // Some deployments use OperatorID for edit-lock checks.
      "OperatorID",
      "VendorID",
      "VendorName",
      "Address1",
      ...
      "ContactPersonName",
      "ProductDetails",
      // Some deployments still require CreatedBy/ModifiedBy even when other audit fields are rejected.
      "CreatedBy",
      "ModifiedBy",
      // Some deployments also require supervision fields.
      "SupervisedBy",
      "NewRecord",
    ]);

    const keepLevel3 = new Set([
      // Level 3: ultra-minimal, required-by-error set (drop BankID/audit timestamps/etc)
      "VendorID",
      "VendorName",
      "Address1",
      "Address2",
  ...
  function applyEditableFormValues(values) {
    const v = values || {};
    if (qs("#Name")) qs("#Name").value = String(v.VendorName ?? "");
