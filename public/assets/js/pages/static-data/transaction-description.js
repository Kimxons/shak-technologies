      ...
      if (results) {
        ...
        results.innerHTML = displayRows.map((r, idx) => {
          return `<tr style="cursor: pointer;">
              ...
              <td class="text-end">
                <button type="button" class="btn btn-sm btn-primary" data-trx-index="${idx}">Select</button>
              </td>
            </tr>`;
        }).join("");

        qsa("tr", results).forEach(tr => {
          tr.addEventListener("click", (e) => {
            const btn = e.currentTarget.querySelector("button[data-trx-index]");
            if (btn) btn.click();
          });
        });

        qsa("button[data-trx-index]", results).forEach(btn => {
          btn.addEventListener("click", (e) => {
            e.stopPropagation();
            const rowIdx = Number(btn.dataset.trxIndex);
