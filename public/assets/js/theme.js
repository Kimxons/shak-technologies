(function(root){
  ...
  async function loadEffectiveThemeOnPageLoad() {
    try {
      if (window.AccountService && typeof window.AccountService.getEffectiveTheme === "function") {
        const themeRequestData = {
          UserID: window.currentUserId || "",
          BranchID: window.currentBranchId || "",
          BankID: window.currentBankId || ""
        };
        
        const response = await window.AccountService.getEffectiveTheme(themeRequestData);
        ...
        if (response && response.Data) {
          ...
          if (themeData.SettingsJson) {
            try {
              ...
              console.log("Effective theme loaded from database:", themSettings);
            } catch (parseError) {
