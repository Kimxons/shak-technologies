...
(function() {
    ...
    const searchDialogConfig = {
        'scheme': {
            url: '../../common/searchDialogs/loan-scheme-search/loan-scheme-search.html',
    ...
    function openSearchDialog(lookupType) {
        ...
        // Build dynamic URL for deposit product searches based on collateral type
        let searchUrl = config.url;
        
        // Handle deposit product searches - apply dynamic filter based on collateral type
        if (lookupType.startsWith('deposit-product-')) {
            const baseUrl = '../../common/searchDialogs/product-search/product-search.html';
            let collateralValue = '';
            
            // Get the corresponding collateral dropdown value
            if (lookupType === 'deposit-product-primary') {
                collateralValue = document.getElementById('PrimaryCollateral')?.value || '';
            } else if (lookupType === 'deposit-product-secondary') {
                collateralValue = document.getElementById('SecondaryCollateral')?.value || '';
            } else if (lookupType === 'deposit-product-additional') {
                collateralValue = document.getElementById('AdditionalCollateral')?.value || '';
            }
            
            // Map collateral dropdown value to ProductTypeID
            // COMSAV → CS, VOLSAV/GRPSAV → SB
            let productTypeFilter = '';
            if (collateralValue === 'COMSAV' || collateralValue === 'CS') {
                // Compulsory Savings
                productTypeFilter = "ProductTypeID='CS' AND BankID ='00' AND ProductCategoryID='A' AND CurrencyID='ETB'";
            } else if (collateralValue === 'VOLSAV' || collateralValue === 'GRPSAV' || collateralValue === 'SB' || collateralValue === 'VS') {
                // Voluntary Savings / Group Savings
                productTypeFilter = "ProductTypeID='SB' AND BankID ='00' AND ProductCategoryID='A' AND CurrencyID='ETB'";
            } else {
                // Default: both SB and CS if no collateral selected
                productTypeFilter = "ProductTypeID in('SB', 'CS') AND BankID ='00' AND ProductCategoryID='A' AND CurrencyID='ETB'";
            }
            
            searchUrl = baseUrl + '?advFilter=' + encodeURIComponent(productTypeFilter);
        }
        
        // Append noheader parameter to hide the search dialog's internal header
        const separator = searchUrl.includes('?') ? '&' : '?';
        searchModalFrame.src = searchUrl + separator + 'noheader=1';
    ...
    async function handleSearchSelection(data) {
        ...
        if (lookupType === 'scheme') {
            ...
            if (idField) {
                // Support various field mappings from different search dialogs
                idField.value = data.LoanSchemeID || data.SchemeProductID || data.WFAdvTypeID || data.WorkflowID || data.GroupProductID || data.ProductID || data.ID || '';
            }
    ...
    function setFormMode(mode) {
        ...
        inputs.forEach(input => {
            ...
            // Handle fields that depend on checkbox state (SavingsCollectionType and SavingsValue)
            if (input.id === 'SavingsCollectionType' || input.id === 'SavingsValue') {
                ...
                if (isViewLikeMode) {
                    // In view modes, disable regardless of checkbox state
                    input.disabled = true;
                } else {
                    // In add/edit modes, enable only if checkbox is checked
    ...
    function validateForm() {
        ...
        // Validate Saving To Loan Ratio
        const savingToLoanRatio = document.getElementById('SavingToLoanRatio')?.value?.trim();
        if (savingToLoanRatio) {
            const numericValue = parseFloat(savingToLoanRatio);
            if (isNaN(numericValue) || numericValue > 99999) {
                showError('Error on Saving To Loan Ratio field');
                document.getElementById('SavingToLoanRatio')?.focus();
                return false;
            }
        }
    ...
    function setCheckboxValue(fieldId, value) {
        ...
        console.log(`Set checkbox ${fieldId} to ${isChecked}`);
   }
    ...
    /**
     * Load and populate dropdown options from API
     */
    async function loadDropdownOptions() {
        try {
            // Load S/L Recovery Type options
            await loadSLRecoveryTypeOptions();
            
            // Load Collateral Type options
            await loadCollateralTypeOptions();
    ...
    /**
     * Load S/L Recovery Type options from API
     */
    async function loadSLRecoveryTypeOptions() {
        try {
            if (!window.LookupService) {
                console.warn('[CenterLoanScheme] LookupService not available');
                return;
            }

            console.log('[CenterLoanScheme] Loading S/L Recovery Type options...');
            const options = await window.LookupService.getSavingPaymentTypes();
            
            if (options && options.length > 0) {
                populateSLRecoveryTypeDropdown(options);
                console.log('[CenterLoanScheme] S/L Recovery Type options loaded:', options.length);
            } else {
                console.warn('[CenterLoanScheme] No S/L Recovery Type options returned');
            }
        } catch (error) {
            console.error('[CenterLoanScheme] Error loading S/L Recovery Type options:', error);
        }
    }

    /**
     * Populate S/L Recovery Type dropdown with options
     */
    function populateSLRecoveryTypeDropdown(options) {
        const dropdown = document.getElementById('SLRecoveryType');
        if (!dropdown) {
            console.warn('[CenterLoanScheme] SLRecoveryType dropdown not found');
            return;
        }
        ...
        console.log('[CenterLoanScheme] Populated S/L Recovery Type dropdown with', options.length, 'options');
    }

    /**
     * Load Collateral Type options from API
     */
    async function loadCollateralTypeOptions() {
        try {
            if (!window.LookupService) {
                console.warn('[CenterLoanScheme] LookupService not available for collateral types');
                return;
            }

            console.log('[CenterLoanScheme] Loading Collateral Type options...');
            
            // Load specific collateral options for each dropdown
            await Promise.all([
                loadPrimaryCollateralOptions(),
                loadSecondaryCollateralOptions(),
                loadAdditionalCollateralOptions()
            ]);
            
            console.log('[CenterLoanScheme] All collateral type options loaded');
            
        } catch (error) {
            console.error('[CenterLoanScheme] Error loading Collateral Type options:', error);
            // Use fallback options if API fails
            loadCollateralFallbackOptions();
        }
    }

    /**
     * Load Primary Collateral options
     */
    async function loadPrimaryCollateralOptions() {
        try {
            const options = await window.LookupService.getSystemCodeOptions('PrimaryCollateralID');
            
            if (options && options.length > 0) {
                populateCollateralDropdown('PrimaryCollateral', options);
                console.log('[CenterLoanScheme] Primary Collateral options loaded:', options.length);
            } else {
                // Fallback options for primary
                populateCollateralDropdown('PrimaryCollateral', [
                    { value: 'COMSAV', label: 'Compulsory Savings' },
                    { value: 'VOLSAV', label: 'Voluntary Savings' }
                ]);
            }
        } catch (error) {
            console.error('[CenterLoanScheme] Error loading Primary Collateral options:', error);
            populateCollateralDropdown('PrimaryCollateral', [
                { value: 'COMSAV', label: 'Compulsory Savings' },
                { value: 'VOLSAV', label: 'Voluntary Savings' }
            ]);
    ...
    /**
     * Load Secondary Collateral options
     */
    async function loadSecondaryCollateralOptions() {
        try {
            const options = await window.LookupService.getSystemCodeOptions('SecondaryCollateralID');
            
            if (options && options.length > 0) {
                populateCollateralDropdown('SecondaryCollateral', options);
                console.log('[CenterLoanScheme] Secondary Collateral options loaded:', options.length);
            } else {
                // Fallback options for secondary
                populateCollateralDropdown('SecondaryCollateral', [
                    { value: 'GRPSAV', label: 'Group Savings' },
                    { value: 'COMSAV', label: 'Compulsory Savings' },
                    { value: 'VOLSAV', label: 'Voluntary Savings' }
                ]);
            }
        } catch (error) {
            console.error('[CenterLoanScheme] Error loading Secondary Collateral options:', error);
            populateCollateralDropdown('SecondaryCollateral', [
                { value: 'GRPSAV', label: 'Group Savings' },
                { value: 'COMSAV', label: 'Compulsory Savings' },
                { value: 'VOLSAV', label: 'Voluntary Savings' }
    ...
    /**
     * Load Additional Collateral options
     */
    async function loadAdditionalCollateralOptions() {
        try {
            const options = await window.LookupService.getSystemCodeOptions('AdditionalCollateralID');
            
            if (options && options.length > 0) {
                populateCollateralDropdown('AdditionalCollateral', options);
                console.log('[CenterLoanScheme] Additional Collateral options loaded:', options.length);
            } else {
                // Fallback options for additional
                populateCollateralDropdown('AdditionalCollateral', [
                    { value: 'GRPSAV', label: 'Group Savings' },
                    { value: 'COMSAV', label: 'Compulsory Savings' },
                    { value: 'VOLSAV', label: 'Voluntary Savings' }
                ]);
            ...
            console.error('[CenterLoanScheme] Error loading Additional Collateral options:', error);
            populateCollateralDropdown('AdditionalCollateral', [
                { value: 'GRPSAV', label: 'Group Savings' },
                { value: 'COMSAV', label: 'Compulsory Savings' },
                { value: 'VOLSAV', label: 'Voluntary Savings' }
    ...
    /**
     * Load fallback options for all collateral dropdowns
     */
    function loadCollateralFallbackOptions() {
        // Primary collateral fallback
        populateCollateralDropdown('PrimaryCollateral', [
            { value: 'COMSAV', label: 'Compulsory Savings' },
            { value: 'VOLSAV', label: 'Voluntary Savings' }
        ]);

        // Secondary and Additional fallback
        const allOptions = [
            { value: 'GRPSAV', label: 'Group Savings' },
            { value: 'COMSAV', label: 'Compulsory Savings' },
            { value: 'VOLSAV', label: 'Voluntary Savings' }
        ];
        populateCollateralDropdown('SecondaryCollateral', allOptions);
        populateCollateralDropdown('AdditionalCollateral', allOptions);
    }

    /**
     * Populate a specific collateral dropdown with options
     */
    function populateCollateralDropdown(dropdownId, options) {
        const dropdown = document.getElementById(dropdownId);
        if (!dropdown) {
            console.warn(`[CenterLoanScheme] ${dropdownId} dropdown not found`);
            return;
        }
        ...
        console.log(`[CenterLoanScheme] Populated ${dropdownId} dropdown with`, options.length, 'options');
    }
    ...
    function initEventListeners() {
        ...
        // Handle Collect Saving With Installment checkbox to enable/disable related fields
        const collectSavingCheckbox = document.getElementById('CollectSavingWithInstallment');
        if (collectSavingCheckbox) {
            collectSavingCheckbox.addEventListener('change', function() {
                ...
                if (this.checked) {
                    ...
                    if (savingsCollectionType) savingsCollectionType.disabled = false;
                    if (savingsValue) savingsValue.disabled = false;
                } else {
                    // Disable and clear fields when unchecked
            ...
            const savingsCollectionType = document.getElementById('SavingsCollectionType');
            const savingsValue = document.getElementById('SavingsValue');
            if (savingsCollectionType) savingsCollectionType.disabled = true;
            if (savingsValue) savingsValue.disabled = true;
        }
        ...
        window.addEventListener('message', function(event) {
            ...
            if (event.data?.type === 'GROUP_LOAN_SCHEME_SELECTED') {
            ...
            } else if (event.data?.type === 'LOAN_SCHEME_SELECTED') {
                // Loan Scheme search dialog format
                handleSearchSelection(event.data.data || {
                    LoanSchemeID: event.data.schemeId,
                    Description: event.data.schemeName
                });
                ...
                const productData = event.data.data || {
                    ...
                    InstallmentFrequency: event.data.installmentFrequency
                };
                
                // Handle the search selection first
                handleSearchSelection(productData);
                
                // Auto-fill InstallmentFrequency if available
                if (event.data.installmentFrequency) {
                    const installmentFrequencyField = document.getElementById('InstallmentFrequency');
                    if (installmentFrequencyField) {
                        installmentFrequencyField.value = event.data.installmentFrequency;
                        console.log(`Auto-filled InstallmentFrequency: ${event.data.installmentFrequency}`);
                    }
                }
