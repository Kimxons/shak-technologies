...
(function () {
    ...
    function wireRecentActivities() {
        // Wire recent activities items (those with sidebar-item--static class, not data-child-form)
    ...
    async function tryGetAccount(direction = 0) {
        ...
        if (!hasData) {
            ...
            showPageLoader(false);
            showErrorMessage(`Account '${payload.AccountID}' not found. Please verify the Account ID and try again.`);
        ...
        if (!bindResult?.ok) {
            showPageLoader(false);
            showErrorMessage(`Account '${payload.AccountID}' not found or invalid. Please verify the Account ID and try again.`);
            window.AccountMaintenanceState.isAccountLoaded = false;
            return;
        }
        ...
        if (!bindResult.boundCount) {
            showPageLoader(false);
            showErrorMessage(`Account '${payload.AccountID}' loaded, but data could not be mapped to form fields. The account may be invalid or corrupted.`);
            window.AccountMaintenanceState.isAccountLoaded = false;
            return;
        }
        ...
        if (currentMode !== 'ADD' && payload.AccountID) {
            ...
            if (!recordAccountID || recordAccountID.trim() === '') {
                ...
                showPageLoader(false);
                showErrorMessage(`Account '${payload.AccountID}' not found. The API response does not contain account data. Please verify the Account ID and try again.`);
                window.AccountMaintenanceState.isAccountLoaded = false;
            ...
            if (normalizedRequested !== normalizedRecord && 
                ...
                showPageLoader(false);
                showErrorMessage(`Account ID mismatch. Requested '${payload.AccountID}' but API returned '${recordAccountID}'. Please verify the Account ID and try again.`);
                window.AccountMaintenanceState.isAccountLoaded = false;
                resetAllFormControls();
                return;
            }
            ...
            if (!recordHasAccountData) {
                ...
                showPageLoader(false);
                showErrorMessage(`Account '${payload.AccountID}' not found or contains no data. Please verify the Account ID and try again.`);
                window.AccountMaintenanceState.isAccountLoaded = false;
                resetAllFormControls();
                return;
            }
            ...
            if (!boundAccountID || boundAccountID === '') {
                ...
                showPageLoader(false);
                showErrorMessage(`The loaded data does not contain a valid Account ID. Please verify the Account ID '${payload.AccountID}' and try again.`);
                window.AccountMaintenanceState.isAccountLoaded = false;
                resetAllFormControls();
                return;
            }
    ...
    function resetToDefaultState() {
        ...
        // Enable AccountID field for View operations
        const accountIdField = document.getElementById('AccountID');
        if (accountIdField) {
            accountIdField.disabled = false;
        }
    ...
    function showConfirmationModal(title, message, onConfirm, onCancel) {
        ...
        const modalHtml = `
            <div class="info-detail-overlay" id="${modalId}">
                <div class="info-detail-modal confirm-modal">
                    <div class="info-detail-header">
                        <div class="info-detail-title">
                            <i class="bi bi-exclamation-triangle"></i>
                            <span>${title}</span>
                        </div>
                        <button class="info-detail-close" aria-label="Close">&times;</button>
                    </div>
                    <div class="info-detail-body">
                        <p>${message}</p>
                    </div>
                    <div class="info-detail-footer">
                        <button class="info-detail-btn info-detail-btn-secondary" data-action="cancel-confirm">
                            <i class="bi bi-x-lg"></i> No, Keep Editing
                        </button>
                        <button class="info-detail-btn info-detail-btn-primary" data-action="confirm">
                            <i class="bi bi-check-lg"></i> Yes, Cancel
                        </button>
                    </div>
                </div>
            </div>
        ...
        modal.querySelector('.info-detail-close').addEventListener('click', () => {
            closeModal();
