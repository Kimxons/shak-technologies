// GL Account Sub Type Module - Event Handlers and Functionality

document.addEventListener('DOMContentLoaded', function() {
  // Initialize module
  initializeGLAccountSubType();
  attachEventListeners();
});

function initializeGLAccountSubType() {
  // Set default values
  document.getElementById('glSubTypeGroupSelect').value = '--Select--';
}

function attachEventListeners() {
  // Left Panel - DataEntry Button
  const dataEntryBtn = document.getElementById('dataEntryBtn');
  if (dataEntryBtn) {
    dataEntryBtn.addEventListener('click', function(e) {
      e.preventDefault();
      toggleDataEntrySubmenu();
    });
  }

  // DataEntry Submenu Item - GL Account Sub Type Nested
  const glAccountSubTypeNestedBtn = document.getElementById('glAccountSubTypeNestedBtn');
  if (glAccountSubTypeNestedBtn) {
    glAccountSubTypeNestedBtn.addEventListener('click', function(e) {
      e.preventDefault();
      openGLAccountSubTypeNestedModal();
    });
  }
...
function openGLAccountSubTypeNestedModal() {
  console.log('Opening GL Account Sub Type Nested modal');
  // Close the submenu
  const submenu = document.getElementById('dataEntrySubmenu');
  if (submenu) {
  ...
  // Find the parent modal and open GL Account Sub Type Nested modal
  try {
    // Get the modal from parent window
    ...
    if (parentModal) {
      ...
      const bootstrapModal = window.parent.bootstrap.Modal.getOrCreateInstance(parentModal);
      bootstrapModal.show();
      
      // Aggressively set z-index after modal is shown
      const setZIndex = () => {
        parentModal.style.zIndex = '99999 !important';
        parentModal.style.position = 'fixed';
        
        // Find and update the modal dialog
        const modalDialog = parentModal.querySelector('.modal-dialog');
        if (modalDialog) {
          modalDialog.style.zIndex = '99999';
        }
        
        // Find and update the backdrop
        const backdrop = window.parent.document.querySelector('.modal-backdrop.show');
        if (backdrop) {
          backdrop.style.zIndex = '99998';
        }
        
        // Update all modal elements
        const allModals = window.parent.document.querySelectorAll('.modal.show');
        allModals.forEach((modal, index) => {
          if (modal.id === 'glAccountSubTypeNestedModal') {
            modal.style.zIndex = '99999';
          }
        });
      };
      
      // Set z-index multiple times to override Bootstrap's auto-management
      setZIndex();
      setTimeout(setZIndex, 50);
      setTimeout(setZIndex, 150);
      setTimeout(setZIndex, 300);
    }
  } catch (e) {
    console.log('GL Account Sub Type Nested modal will be opened', e);
  }
}
...
function handleBackAction() {
  ...
  const modal = document.closest ? document.querySelector('.modal') : null;
  if (modal) {
